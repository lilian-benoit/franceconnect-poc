...
public class FcConnection {
	...
	public URI getRedirectUri() throws FcConnectException {
		...
		try {
			OAuthClientRequest request = OAuthClientRequest
					...
					.setParameter(configuration.getVerifParameterId(),
							configuration.getVerifParameterValue())
					.buildQueryMessage();
	...
	public OAuthAccessTokenResponse getAccessToken(HttpServletRequest request)
		...
		try {
			...
			// vérification a rajouter par rapport au state pour éviter le man
			// in the middle
			// if (StringUtils.isBlank(stateResponse) ||
			// !stateResponse.equals(stateFromSession)) {
			//
			// res =
			// Response.ok("<p>Erreur de verif state response !!</p>",TEXT_HTML);
			...
			throw new FcConnectException(
					"Error during request for accessToken : ", e);
		} catch (OAuthProblemException e) {
			throw new FcConnectException(
					"Error during accessToken retrieving : ", e);
		}
	}
	...
	public String getUserInfo(String accessToken) throws FcConnectException {
		...
		try {
			bearerClientRequest = new OAuthBearerClientRequest(
					configuration.getUserInfoUri()).setAccessToken(accessToken)
					.buildHeaderMessage();
			...
			throw new FcConnectException(
					"Error during userInfo request building : ", e);
		} catch (OAuthProblemException e) {
			throw new FcConnectException("Error during userInfo retrieving : ",
					e);
		}

	}
	public OAuthAccessTokenResponse getDataByAccessToken(HttpServletRequest request)
			throws FcConnectException {
